...
//inputEl是定位到完成复选框的元素变量
        inputEl.addEventListener("change", (e) => {
            todoItems[i].isFinishedTime = new Date().toLocaleDateString() + " " +new Date().toLocaleTimeString('chinese',{hour12:false})
...
//由于his页面和todolist仅仅只是切换了一个视角，而并未切换页面，所以此处的类名仍可以定义为todoitem方便！！！！！！！！！！
        ...
        FinishedTime.className = "FinishedTime-flag";

        FinishedTime.innerText = item. isFinishedTime
        
        titleEl.innerText = item.title;

        itemDiv.append(titleEl);
        itemDiv.append(importanceEl);
        itemDiv.append(FinishedTime);
...
function renderInputPane(todoItems) {//位于input区域的所有控制和触发
    ...
    addBtnEl.addEventListener("click", (e)=>{
        ...
        inputEl.value = null  //在输入完成之后，将inputEl中的值删去，方便页面使用者自行删除不需要的内容
...
function ChangeTo1(){
    document.body.style.backgroundImage ="URL(1.jfif)";
}
function ChangeTo2(){
    document.body.style.backgroundImage ="URL(2.jfif)";
}
function ChangeTo3(){
    document.body.style.backgroundImage ="URL(3.jfif)";
}
function ChangeTo4(){
    document.body.style.backgroundImage ="URL(4.jfif)";
}
